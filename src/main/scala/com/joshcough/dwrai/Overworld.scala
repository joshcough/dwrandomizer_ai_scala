...
object Overworld {
  ...
  val OVERWORLD_TILES: IndexedSeq[OverworldTile] =
    IndexedSeq(Grass,Desert,Hills,Mountain,Water,Stone,Forest,Swamp,Town,Cave,Castle,Bridge,Stairs)
    ...
    def getOverworldTileRow(overworldPointer: Int): IndexedSeq[OverworldTile] = {
      IndexedSeq.unfold((0, overworldPointer)) { case (totalCount, currentAddr) =>
        if (totalCount < 120) {
          ...
          Some((Range(0, count).map(_ => OVERWORLD_TILES(tileId)), (totalCount + count, currentAddr + 1)))
        }
        else None
      }.flatten
    }
